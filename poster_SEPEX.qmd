...
plot_1000 <- plot_trial(sub1000[[example_sub_nr]], trial_nr = example_trial_nr, 
           show_original_samples = TRUE,show_downsampled_samples = FALSE,downsampling_rate = 1000,show_eyelink_fixations = TRUE, show_EK_fixations = FALSE, show_EK_fixations_downsampled = FALSE,show_words = TRUE,show_word_boundaries = TRUE,x_limits = c(90,1400), y_limits = c(650,450), letter_width = 12, words_font_size = 3) + theme_classic()
...
example_sub_nr_for_250 <- 8

example_itemN_for_250 <- sub1000[[example_sub_nr]]$trial_db[example_trial_nr,]$itemN

example_cond_for_250 <- sub1000[[example_sub_nr]]$trial_db[example_trial_nr,]$cond
...
# for(i in 1:32){
#    cat(i, ": ", which(sub250[[i]]$trial_db$itemN == example_itemN_for_250 & sub250[[i]]$trial_db$cond == example_cond_for_250), ", ", sep = "")}
...
plot_1000_downsampled <- plot_trial(sub1000[[example_sub_nr]], trial_nr = example_trial_nr, 
           show_original_samples = FALSE,show_downsampled_samples = TRUE,downsampling_rate = 50,show_eyelink_fixations = FALSE, show_EK_fixations = FALSE, show_EK_fixations_downsampled = TRUE,show_words = TRUE,show_word_boundaries = TRUE,x_limits = c(90,1400), y_limits = c(650,450), letter_width = 12, words_font_size = 3) + theme_classic()
...
brmplot_gd_downsampled_average_100 <- brm_EK_gd_downsampled_average_100 %>% filter(.variable == "b_condition1") %>%
  ...
  labs(title = "Posterior distribution for frequency effect coefficient", subtitle = "log(FFD) on the target word", x = "Value", y = "Sampling rate (downsampled by averaging, Hz)", caption = "Coefficient can be interpreted as the difference between high and low frequency target words.\nRed box gives proportion of posterior distribution < 0") + theme(legend.position = "none")
...
brmplot_gd_downsampled_average_all <- brm_EK_gd_downsampled_average_all %>% 
  filter(.variable == "b_condition1") %>%   
  ggplot(aes(y = factor(rate), x = .value, fill = after_stat(x > 0))) +   geom_vline(xintercept = 0, color = "gray", linetype = "dashed") +   stat_halfeye() +    geom_label(data = brm_EK_gd_downsampled_average_all %>%               filter(.variable == "b_condition1") %>%               group_by(rate) %>%               summarize(proportion_below_zero = mean(.value < 0)),             aes(label = sprintf("%.2f", proportion_below_zero), x = 0, y = factor(rate)),             hjust = 0.6, vjust = -1.5, color = "black", size = 4) +    labs(title = "Posterior distribution for frequency effect coefficient", subtitle = "log(FFD) on the target word", x = "Value", y = "Sampling rate (downsampled by averaging, Hz)", caption = "Coefficient can be interpreted as the difference between high and low frequency target words.\nRed box gives proportion of posterior distribution < 0") + theme(legend.position = "none")
