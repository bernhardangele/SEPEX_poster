...
format:
  poster-typst:
    ...
    footer-text: "SEPEX 2024 - October 2024, Almería" # Add your conference name and date
...
plot_1000 <- plot_trial(sub1000[[example_sub_nr]], trial_nr = example_trial_nr, 
           show_original_samples = TRUE,show_downsampled_samples = FALSE,downsampling_rate = 1000,show_eyelink_fixations = TRUE, show_EK_fixations = FALSE, show_EK_fixations_downsampled = FALSE,show_words = TRUE,show_word_boundaries = TRUE,x_limits = c(90,1100), y_limits = c(650,450), letter_width = 12, words_font_size = 3) + theme_classic()
...
# this subject must have the same trials at 250 Hz that the example subject had at 1000 Hz
example_sub_nr_for_250 <- 11

example_itemN_for_250 <- sub1000[[example_sub_nr]]$trial_db[example_trial_nr,]$itemN

trial_nr_for_250 <- which(sub250[[example_sub_nr_for_250]]$trial_db$itemN == example_itemN_for_250)

# help for finding subjects who have this trial
#for(i in 1:32){
#   cat(i, ": ", which(sub250[[i]]$trial_db$itemN == example_itemN_for_250), ", ", sep = "")}

if (length(trial_nr_for_250) == 0) {
  trial_nr_for_250 = 1
  warning("Trial not found in 250 Hz data for this subject, using first trial instead")
}

plot_250 <- plot_trial(sub250[[example_sub_nr_for_250]], trial_nr = trial_nr_for_250, 
           show_original_samples = TRUE,show_downsampled_samples = FALSE,downsampling_rate = 250,show_eyelink_fixations = TRUE, show_EK_fixations = FALSE, show_EK_fixations_downsampled = FALSE,show_words = TRUE,show_word_boundaries = TRUE,x_limits = c(90,1100), y_limits = c(650,450), letter_width = 12, words_font_size = 3) + theme_classic()
...
plot_1000_downsampled <- plot_trial(sub1000[[example_sub_nr]], trial_nr = example_trial_nr, 
           show_original_samples = FALSE,show_downsampled_samples = TRUE,downsampling_rate = 50,show_eyelink_fixations = FALSE, show_EK_fixations = FALSE, show_EK_fixations_downsampled = TRUE,show_words = TRUE,show_word_boundaries = TRUE,x_limits = c(90,1100), y_limits = c(650,450), letter_width = 12, words_font_size = 3) + theme_classic()
...
#| message: false
#| warning: false
#| fig.cap: "Evidence for frequency effect in GD on the target word by sampling rate (downsampled data, 100 sentences per participant)"
#| fig.align: "center"
#| fig.width: 9
#| fig.height: 5


brm_EK_gd_125_average_100 <- qread("../../brm_EK_gd_125_average_100_items_per_sub.qs")

brm_EK_gd_50_average_100 <- qread("../../brm_EK_gd_50_average_100_items_per_sub.qs")

brm_EK_gd_30_average_100 <- qread("../../brm_EK_gd_30_average_100_items_per_sub.qs")


brm_EK_gd_downsampled_average_100 <- rbind(brm_EK_gd_30_average_100 %>% gather_draws(b_condition1) %>% mutate(rate = 31.25),
                        brm_EK_gd_50_average_100 %>% gather_draws(b_condition1) %>% mutate(rate = 50),
                        brm_EK_gd_125_average_100 %>% gather_draws(b_condition1) %>% mutate(rate = 125)) 

brmplot_gd_downsampled_average_100 <- brm_EK_gd_downsampled_average_100 %>% filter(.variable == "b_condition1") %>%
  ggplot(aes(y = factor(rate), x = .value, fill = after_stat(x > 0))) +
  geom_vline(xintercept = 0, color = "gray", linetype = "dashed") +
  stat_halfeye() + 
  geom_label(data = brm_EK_gd_downsampled_average_100 %>%
              filter(.variable == "b_condition1") %>%
              group_by(rate) %>%
              summarize(proportion_below_zero = mean(.value < 0)),
            aes(label = sprintf("%.2f", proportion_below_zero), x = 0, y = factor(rate)),
            hjust = 0.6, vjust = -1.5, color = "black", size = 4) + 
  labs(title = "Posterior distribution for frequency effect coefficient", subtitle = "log(GD) on the target word", x = "Value", y = "Sampling rate (downsampled by averaging, Hz)", caption = "Coefficient can be interpreted as the difference between high and low frequency target words.\nRed box gives proportion of posterior distribution < 0") + theme(legend.position = "none")

brmplot_gd_downsampled_average_100
```
...
#| message: false
#| warning: false
#| fig.cap: "Evidence for frequency effect in GD on the target word by sampling rate (downsampled data, 400 sentences per participant)"
#| fig.align: "center"
#| fig.width: 9
#| fig.height: 5


brm_EK_gd_125_average_all <- qread("../../brm_EK_gd_125_average_all_sentences.qs")
brm_EK_gd_50_average_all <- qread("../../brm_EK_gd_50_average_all_sentences.qs")
brm_EK_gd_30_average_all <- qread("../../brm_EK_gd_30_average_all_sentences.qs") 
brm_EK_gd_downsampled_average_all <- rbind(brm_EK_gd_30_average_all %>% 
                                              gather_draws(b_condition1) %>% 
                                              mutate(rate = 31.25),
                                            brm_EK_gd_50_average_all %>% 
                                              gather_draws(b_condition1) %>% 
                                              mutate(rate = 50),
                                            brm_EK_gd_125_average_all %>% 
                                              gather_draws(b_condition1) %>% 
                                              mutate(rate = 125))
brmplot_gd_downsampled_average_all <- brm_EK_gd_downsampled_average_all %>% 
  filter(.variable == "b_condition1") %>%   
  ggplot(aes(y = factor(rate), x = .value, fill = after_stat(x > 0))) +   geom_vline(xintercept = 0, color = "gray", linetype = "dashed") +   stat_halfeye() +    geom_label(data = brm_EK_gd_downsampled_average_all %>%               filter(.variable == "b_condition1") %>%               group_by(rate) %>%               summarize(proportion_below_zero = mean(.value < 0)),             aes(label = sprintf("%.2f", proportion_below_zero), x = 0, y = factor(rate)),             hjust = 0.6, vjust = -1.5, color = "black", size = 4) +    labs(title = "Posterior distribution for frequency effect coefficient", subtitle = "log(GD) on the target word", x = "Value", y = "Sampling rate (downsampled by averaging, Hz)", caption = "Coefficient can be interpreted as the difference between high and low frequency target words.\nRed box gives proportion of posterior distribution < 0") + theme(legend.position = "none")
brmplot_gd_downsampled_average_all
```
